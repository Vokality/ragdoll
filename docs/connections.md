...
- These are ordinary function tools in Lumen's existing execution loop. Every invocation and result therefore uses the existing durable tool history, cancellation, multi-call continuation, and unknown-outcome handling. MCP requests execute in the Electron main process; connection credentials are not included in model requests, model prompts, chat history, or renderer snapshots.
