  ...
  return [
    {
      ...
      handler: (args, _ctx) => handler.setMood(args as unknown as SetMoodArgs),
...
export interface CharacterExtensionOptions {
  /** Handler that executes the character tools */
  handler: CharacterToolHandler;
  /** Optional extension ID override (default: "character") */
  id?: string;
}

/**
 * Create a character extension with the provided handler.
 *
 * @example
 * ```ts
 ...
 *       controller.setMood(mood, duration);
 *       return { success: true };
 *     },
 *     triggerAction: async ({ action, duration }) => {
 *       controller.triggerAction(action, duration);
 *       return { success: true };
 *     },
 *     setHeadPose: async ({ yawDegrees, pitchDegrees, duration }) => {
 *       controller.setHeadPose(yawDegrees, pitchDegrees, duration);
 *       return { success: true };
 *     },
 *     setSpeechBubble: async ({ text, tone }) => {
 *       controller.setSpeechBubble(text, tone);
 *       return { success: true };
 *     },
 *   },
 * });
...
export function createCharacterExtension(
  ...
  const { handler, id = "character" } = options;
