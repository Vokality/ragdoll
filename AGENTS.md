...
`.cursor/run-lumen-headless.sh` avoids both: it launches Electron inside a private D-Bus session (which has no GUI keyring prompter, so no dialog is possible) with a dedicated, auto-unlocked `gnome-keyring` in its own `XDG_DATA_HOME` and `--password-store=gnome-libsecret`. It never touches the desktop `login` keyring. The same launcher passes `--ignore-gpu-blocklist` and SwiftShader WebGL flags so the Three.js character can render on the virtual GPU Chromium would otherwise blocklist. Always launch the cloud app through this script; do not fold the keyring or GPU workarounds into the cross-platform `dev:chat` script, where the OS keychain already backs `safeStorage`.
