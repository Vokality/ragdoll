...
function renderGradients(theme: RagdollTheme): React.JSX.Element {
  const renderGradient = (id: string, gradient: GradientDef): React.JSX.Element => {
    if (gradient.type === 'linear') {
      return (
        <linearGradient key={id} id={id} x1={gradient.x1} y1={gradient.y1} x2={gradient.x2} y2={gradient.y2}>
      ...
      return (
        <radialGradient key={id} id={id} cx={gradient.cx} cy={gradient.cy} r={gradient.r}>
          {gradient.stops.map((stop, idx) => (
            <stop key={idx} offset={stop.offset} stopColor={stop.stopColor} />
          ))}
        </radialGradient>
...
interface RenderData {
  ...
  leftEarPath: string;
  rightEarPath: string;
  leftEyePaths: ReturnType<ReturnType<CharacterController['getGeometry']>['getEyePath']>;
  rightEyePaths: ReturnType<ReturnType<CharacterController['getGeometry']>['getEyePath']>;
  leftIris: ReturnType<ReturnType<CharacterController['getGeometry']>['getIrisPosition']>;
  rightIris: ReturnType<ReturnType<CharacterController['getGeometry']>['getIrisPosition']>;
  leftEyebrowPath: string;
  rightEyebrowPath: string;
  mouthPaths: ReturnType<ReturnType<CharacterController['getGeometry']>['getMouthPath']>;
  currentTheme: RagdollTheme;
}

function computeRenderData(controller: CharacterController, theme?: RagdollTheme): RenderData {
  ...
  const expression = applyIdleToExpression(baseExpression, idleState.blinkAmount, idleState.pupilOffsetX, idleState.pupilOffsetY);
...
export function RagdollCharacter({ onControllerReady, theme }: RagdollCharacterProps) {
  ...
  return (
    <svg
      ...
      style={{ width: '100%', height: '100%', maxWidth: '320px', maxHeight: '380px' }}
    >
      ...
      {ht.shadowIntensity > 0.02 && (
        <ellipse
          cx={ht.shadowSide === 'left' ? -dims.headWidth / 3 : dims.headWidth / 3}
          cy={0}
          rx={30}
          ry={dims.headHeight / 2 - 15}
          fill={ht.shadowSide === 'left' ? 'url(#faceShadowLeft)' : 'url(#faceShadowRight)'}
      ...
      <g transform={`translate(${ht.mouthShiftX}, ${ht.mouthShiftY})`}>
        {/* Mouth opening (dark) */}
        {mouthPaths.opening && (
          <path d={mouthPaths.opening} fill={currentTheme.colors.shadow.color.replace('0.3', '0.8')} />
        )}
        ...
        {expression.mouth.lowerLipTop - expression.mouth.upperLipBottom > 6 && (
          <rect
            ...
            height={Math.min(8, (expression.mouth.lowerLipTop - expression.mouth.upperLipBottom) * 0.6)}
            fill="#f8f8f0"
            rx={2}
          />
        )}
